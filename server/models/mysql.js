...
var queries = { //list of mysql queries
	...
	getUserTickets: "SELECT tickets.id, tickets.creator_id, tickets.assigned_id, UNIX_TIMESTAMP(tickets.created_at) AS 'created_at', tickets.title, tickets.status, tickets.discover_phase, tickets.priority, (SELECT UNIX_TIMESTAMP(ticket_comments.created_at) FROM ticket_comments WHERE ticket_comments.ticket_id = tickets.id AND ticket_comments.creator_id = tickets.creator_id ORDER BY ticket_comments.id DESC LIMIT 1) AS 'latest_comment_timestamp', (SELECT ticket_comments.comment FROM ticket_comments WHERE ticket_comments.ticket_id = tickets.id ORDER BY ticket_comments.id DESC LIMIT 1) AS 'latest_comment', (SELECT users.username FROM ticket_comments JOIN users ON (ticket_comments.creator_id = users.id) WHERE ticket_comments.ticket_id = tickets.id ORDER BY ticket_comments.id DESC LIMIT 1) AS 'latest_comment_author' FROM tickets WHERE tickets.creator_id = ?",
	...
	getTickets: "SELECT tickets.id, tickets.creator_id, tickets.assigned_id, UNIX_TIMESTAMP(tickets.created_at) AS 'created_at', tickets.title, tickets.status, tickets.discover_phase, tickets.priority, (SELECT UNIX_TIMESTAMP(ticket_comments.created_at) FROM ticket_comments WHERE ticket_comments.ticket_id = tickets.id AND ticket_comments.creator_id = tickets.creator_id ORDER BY ticket_comments.id DESC LIMIT 1) AS 'latest_comment_timestamp', (SELECT ticket_comments.comment FROM ticket_comments WHERE ticket_comments.ticket_id = tickets.id ORDER BY ticket_comments.id DESC LIMIT 1) AS 'latest_comment', (SELECT users.username FROM ticket_comments JOIN users ON (ticket_comments.creator_id = users.id) WHERE ticket_comments.ticket_id = tickets.id ORDER BY ticket_comments.id DESC LIMIT 1) AS 'latest_comment_author' FROM tickets",
