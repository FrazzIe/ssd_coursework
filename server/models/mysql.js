...
var queries = { //list of mysql queries
	...
	getTickets: "SELECT tickets.id, tickets.creator_id, tickets.assigned_id, UNIX_TIMESTAMP(tickets.created_at) AS 'created_at', tickets.title, tickets.status, tickets.discover_phase, tickets.priority, (SELECT UNIX_TIMESTAMP(ticket_comments.created_at) FROM ticket_comments WHERE ticket_comments.ticket_id = tickets.id AND ticket_comments.creator_id = tickets.creator_id ORDER BY id DESC LIMIT 1) AS 'latest_comment_timestamp' FROM tickets",
