...
var queries = { //list of mysql queries
	...
	getTicketById: "SELECT tickets.id, tickets.creator_id, tickets.assigned_id, UNIX_TIMESTAMP(tickets.created_at) AS 'created_at', tickets.title, tickets.status, tickets.discover_phase, tickets.priority FROM tickets WHERE tickets.id = ?",
	getTicketComments: "SELECT UNIX_TIMESTAMP(ticket_comments.created_at) AS 'created_at', ticket_comments.comment, users.username, users.group FROM ticket_comments JOIN users ON (ticket_comments.creator_id = users.id) WHERE ticket_comments.ticket_id = ?",
